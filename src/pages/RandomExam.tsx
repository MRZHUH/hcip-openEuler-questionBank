    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 p-6">
      <div className="max-w-4xl mx-auto">
        <div className="backdrop-blur-xl bg-white/70 rounded-2xl shadow-soft border border-white/20 p-8 mb-8">
          <h1 className="text-3xl font-extrabold text-gray-800 tracking-tight">随机出题模式</h1>
          <p className="text-gray-600 mt-2">单选{TARGETS.single}×{POINTS.single}分，多选{TARGETS.multiple}×{POINTS.multiple}分，判断{TARGETS.judge}×{POINTS.judge}分，填空{TARGETS.fill}×{POINTS.fill}分，总分1000</p>
          <div className="mt-3 text-sm text-gray-700">实际抽取：单选{totals.actual.single}，多选{totals.actual.multiple}，判断{totals.actual.judge}，填空{totals.actual.fill}</div>
        ...
        {currentQ && (
          <div className="backdrop-blur-xl bg-white/70 rounded-2xl shadow-soft border border-white/20 p-8 mb-8">
            ...
            <h2 className="text-2xl font-semibold text-gray-900 mb-6 leading-relaxed">{currentQ.question}</h2>
            ...
            {currentQ.type !== 'fill' && (
              <div className="space-y-3">
                {currentQ.options.map((option: string, index: number) => {
                  ...
                  return (
                    <button key={index} onClick={() => !showResult && handleAnswer(optionLetter)} disabled={showResult} className={`group w-full p-4 rounded-xl border-2 text-left transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg ${bgColor} ${borderColor} ${textColor} ${!showResult ? 'cursor-pointer' : 'cursor-default'}`}> 
            ...
            {currentQ.type === 'fill' && (
              <div className="space-y-4">
                <input type="text" value={currentAnswers[0] || ''} onChange={(e) => !showResult && handleFillAnswer(e.target.value)} disabled={showResult} placeholder="请输入答案..." className="w-full p-4 border-2 border-gray-300 rounded-lg bg-white/90 focus:border-indigo-500 focus:outline-none text-lg shadow-sm" />
              </div>
            )}
        ...
        <div className="backdrop-blur-xl bg-white/70 rounded-2xl shadow-soft border border-white/20 p-6">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-gray-800 mb-2">成绩统计</h3>
